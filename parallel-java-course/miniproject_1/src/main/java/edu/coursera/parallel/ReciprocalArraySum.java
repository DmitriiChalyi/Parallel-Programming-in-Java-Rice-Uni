...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (endIndexExclusive - startIndexInclusive <= 10000) {
                ...
                int middle = (endIndexExclusive + startIndexInclusive)/2 +1;
    ...
    protected static double parManyTaskArraySum(final double[] input,
                                                final int numTasks) {
        double sum = 0;
        ArrayList<ReciprocalArraySumTask> reciprocalArraySumTasks = new ArrayList<>();
      for (int i=0; i<numTasks; i++) {
          int start = getChunkStartInclusive(i, numTasks, input.length);
          int end = getChunkEndExclusive(i, numTasks, input.length);
          reciprocalArraySumTasks.add(new ReciprocalArraySumTask(start, end, input));
      }
      ForkJoinTask.invokeAll(reciprocalArraySumTasks);
      for(ReciprocalArraySumTask r : reciprocalArraySumTasks) {
          sum += r.getValue();
      }
